...
public class LegendTestPortView extends TestCase
{
	...
	public void testLegentPortView() throws Exception
	{
		...
		WebRequest request = new GetMethodWebRequest(
				m_testUtils.getUrlPmgraph()
						+ "index.jsp?report=totals&graph=cumul&start=0&end=300000&resultLimit=15&view=PORT");
	...
	public void testSorterPortView() throws Exception
	{
		...
		request = new GetMethodWebRequest(
				m_testUtils.getUrlPmgraph()
						+ "index.jsp?start=0&end=300000&sortBy=downloaded&order=ASC&resultLimit=15&view=PORT");
		...
		request = new GetMethodWebRequest(
				m_testUtils.getUrlPmgraph()
						+ "index.jsp?start=0&end=300000&sortBy=uploaded&order=DESC&resultLimit=15&view=PORT");
		response = wc.getResponse(request);
		link = response.getLinkWithName("uploaded");
		sortLink = "/pmgraph/index.jsp?start=0&end=300000&sortBy=uploaded&order=ASC&resultLimit=15&view=PORT";
		assertEquals("Compare the sort link.", sortLink, link.getURLString());
			...
			SQLException, SAXException
	{
		...
		WebRequest request = new GetMethodWebRequest(
				m_testUtils.getUrlPmgraph()
						+ "index.jsp?report=totals&graph=cumul&start=0&end=300000&resultLimit=1&view=PORT");
		WebResponse response = wc.getResponse(request);
