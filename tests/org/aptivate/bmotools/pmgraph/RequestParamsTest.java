...
public class RequestParamsTest extends TestCase
{
	 ...
	 * @throws Exception
	 */
	private RequestParams getRequestParamsFromMockRequest(HashMap<String, String> hashMap)
			throws Exception
	{
		// create mock request
	...
	// TODO check why we are using xml
	/**
	...
	{
		m_doc = m_reader.read(new File(
				"./tests/org/aptivate/bmotools/pmgraph/testRequestParamsData.xml"));
		Element rootData = m_doc.getRootElement();
		int listIndex = 1;
		for (Iterator i = rootData.elementIterator("servletRequest"); i.hasNext(); listIndex++)
		{
			...
			for (Iterator j = request.elementIterator(); j.hasNext();)
			{
			...
			for (Iterator j = expectResult.elementIterator(); j.hasNext();)
			{
				Element reqData = (Element) j.next();
	 ...
	 * @throws Exception
	 */
	private void checkEachParams(RequestParamsTestUtil expectObj, RequestParams requestParams,
